...
### 4.6 台灣供應鏈（15 項，獨立計分，不進綜合溫度）

`tw.items` 共十五項，其中**十四項**經 `tw.subs` 五個子群再加權成 `tw.heat`（`tsmc_weight` 不入子群，見下）：

| 子群 | 權重 | 成員 |
|---|---|---|
| 動能 | 0.25 | `tsmc_200dma`、`tsmc_52w`、`elec_rel`、`twii_pos` |
| 估值 | 0.25 | `tsmc_pe`、`odm_pe` |
| 籌碼 | 0.15 | `tw_margin`、`tw_daytrade`、`short_ratio` |
| 槓桿 | 0.15 | `margin_mktcap`、`margin_keeprate`、`credit_turnover` |
| 基本面 | 0.20 | `tw_rev`、`tw_export` |

**籌碼與槓桿是刻意分開的兩組**：籌碼量的是**行為**（融資的 20 日變動、當沖占比、券資比），
槓桿量的是**水位**（融資占市值、維持率、信用交易占比）。2026-09-24 接入當天這兩組就在講相反的故事
——行為面 73.7（偏熱），水位面 20.5（歷史低檔）——混在同一組會互相抵消，而那個抵消掉的差異
正是這一輪台股行情最該被看見的特徵：它不是融資推上去的。
...
| `margin_mktcap` | 融資餘額占市值 %（正向） | `[[0.30,0],[0.42,33],[0.55,67],[0.80,100]]` |
| `margin_keeprate` | **整體融資維持率取負** `-keep` | `[[-210,0],[-190,25],[-166,60],[-150,85],[-140,100]]` |
| `credit_turnover` | 信用交易占成交總值 %（正向） | `[[5.0,0],[7.0,33],[9.0,67],[12.0,100]]` |
| `short_ratio` | **券資比取負** `-sr` | `[[-8,0],[-5,30],[-3,60],[-1.8,85],[-1.0,100]]` |
...
**每月只有 `tsmc_weight` 需要人更新**（TAIFEX 擋機器人，Actions 端固定失敗），其餘十四項由引擎自動更新（`credit_turnover` 為月頻，其餘每交易日），屬於 §8.3 覆核不動的欄位。

#### 2026-09-24 接入的四項（證交所臺股儀表板端點）

來源是 `https://www.twse.com.tw/dashboard/zh/credit/margin.html` 背後的三支端點：
`MI_MARGN_TREND`（一次回 60 個交易日的融資餘額／張數、融券張數、市值；`days` 實測被夾在 60）、
`BFIJ3U`（整體融資維持率與追繳／處分戶數）、`MI_MARGN_HISTORY`（2000 年以來的年度
融資占市值比重與信用交易占成交總值比重）。

**這四項的錨點有三種不同的依據，不要混為一談：**

| 指標 | 錨點依據 | 可信度 |
|---|---|---|
| `margin_mktcap` | 2018 年以來實際區間 0.36–0.54% | 有長序列，但**刻意只用近段** |
| `credit_turnover` | 近年實際區間 5.78–6.89% | 同上 |
| `margin_keeprate` | **制度常數**：130% 法定追繳線、166% 市場慣用警戒 | 不隨年份漂移，最穩 |
| `short_ratio` | 台股券資比長期落在 1–8% 的認知 | ⚠ **待校準**，本表信心最低 |

**為什麼不用 2000 年以來的全區間當尺。** 融資餘額占市值比重自 2000 年的 2.31% 單向衰減到
2025 年的 0.36%，信用交易占成交總值自 40.98% 降到 5.78%。那是法人化、ETF 化與借券取代融券的
**結構性**變化，不是泡沫程度降了六倍。拿全區間訂錨，今天（0.38%／6.09%）會永遠打個位數、
指標從此不會動——**一根看起來活著的死指標比沒有這根更糟**，因為它會在畫面上佔一格綠燈，
讓人以為量過了。所以兩者都用近段區間訂錨，2000 年那兩個數字只寫進 `disp` 與 `note` 當對照。
代價與 `tw_daytrade` 相同：錨點會漂，要定期回頭校準。

**`short_ratio` 為什麼標待校準。** `MI_MARGN_TREND` 只回 60 個交易日，
`BFIJ3U` 只保留約一個月（2026-09-24 實測 09-08 之後可得、07-01 已無，且沒有往回撈的參數），
所以這兩支都算不出分位數。`margin_keeprate` 靠制度線繞過了這個問題，`short_ratio` 沒有對應的制度線
——它現在這組錨點是專家判斷，**累積滿一年序列後應重訂**。引擎已在 `tw.keeprate_hist` 每天存一筆
維持率與追繳／處分戶數（上限 250 筆）供日後校準用。

**`credit_turnover` 刻意是月頻、而且不自己算。** 2026-09-24 實測：用 `BFIJ3U` 的 `crdAmt`
除以 `FMTQIK` 的當日成交金額得到約 13%，而證交所同期公布的官方值是 6.09%——差一倍，
官方口徑應含雙邊計算。自己推導會在頁面上印出一個跟來源打架的數字，所以放棄日頻、
直接讀 `MI_MARGN_HISTORY` 的最後一列。`margin_mktcap` 則相反：自算的 0.385% 與官方月值 0.38%
對得上，所以維持日頻。

**`margin_keeprate` 是本監控唯一直接量到台股強制賣壓的一項。** 其餘十四項量的都是價格、
估值、成交行為或營收，沒有一項看得到「部位被迫平倉」。維持率越低代表融資戶離追繳越近，
故取負後計分。
