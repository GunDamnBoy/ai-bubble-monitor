...
def main():
    ...
    def f_gsy():
        ...
        upd("gsy_runup", round(g["ret24"], 1), f"{g['ret24']:+.0f}%（24個月）",
            ...
            sub="文獻 53%／80% 為引用，非本標的校準——說明見卡片下方")
        # note 對自動指標本來只放結構性說明、由 data.json 的種子值帶著；這一項改由引擎寫，
        ...
        IND["gsy_runup"]["note"] = (
            ...
            "≥40% 的佔 37.5%（收攏成 5 個獨立事件則為 60%），相對無條件基準 14% 確實有鑑別力——"
            "但背後只有 2 個獨立的崩盤事件，是方向一致而非統計顯著。照論文三條件計算，"
            "SOXX 25 年來只成立過 2 次、150% 門檻 0 次。"
            "**而且這條序列本身不是一個固定的東西**：SOXX 於 2021-06-21 把標的指數由 "
            "PHLX Semiconductor Sector（^SOX）換成 ICE／NYSE Semiconductor（SEC 497 申報），"
            "^SOX 自己也於 2024-04-22 把權重上限由「前五大各 8%」改為「前三大 12%／10%／8%」。"
            "兩者的 24 月漲幅差距在 2003–2020 中位 0.7pp，2024 起 3pp，2025–26 達 9–10pp。"
            "**跨 2021 或跨 2024 的比較不是同一籃股票**——趨勢可看，但那不是同一把尺。")
        if "accel" in g:
